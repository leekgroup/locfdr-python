...
def locmle(z, xlim = None, Jmle = 35, d = 0., s = 1., ep = 1/100000., sw = 0, Cov_in = None):
    ...
    for j in xrange(Jmle):
    	...
    	u1 = np.power(s, range(5))
    	II = np.power(r, np.matrix([[max(j-i, 0) for i in range(5)] for j in range(5)]))
    	I = np.multiply(np.multiply(I, II), u1.transpose())
    	E = np.array(I * np.matrix(H).transpose()).transpose()
    	mu = np.array(E[1], E[2])
    	V = np.matrix([[E[2] - E[1]*E[1], E[3] - E[1] * E[2]],[E[3] - E[1] * E[2], E[4] - E[2]*E[2]])
    	addbet = np.linalg.solve(V, (Y - mu).transpose()).transpose()/(1+1./(j*j))
    	bett = bet + addbet
    	if bett[1] > 0:
    		bett = bet + .1 * addbet
    	if bett[1] >= 0:
    	...
    	if np.sqrt(sum(np.array(np.power(bett - bett, 2))) < ep:
    		break
    if bett[1] >= 0:
    	mle = [None, 6]
    	Cov_lfdr = None
    	Cor = np.matrix([[None]*3]*3)
    else:
    	aa = (aorig - d) / s
    	bb = (borig - d) / s
    	H0 = stats.norm.cdf(bb) - stats.norm.cdf(aa)
		...
		matt = np.array([1/H0] + (-(p0/s) * h).tolist())
		matt = np.matrix([[0]*3]*3)
		matt[1,1] = 1
		matt[2,2] = 1
		matt[0,:] = h
		...
		if sw == 1:
			...
			Cor = pd.DataFrame(Cor, index=['d', 'p0','s'], columns=['d','p0','s'])
...
def loccov(N, N0, p0, d, s, x, X, f, JV, Y, i0, H, h, sw):
	M = numpy.array([1, x - Y[0], x*x - Y[1]]).transpose()
	if sw == 2:
		K = len(x)
		K0 = len(i0)
		mat = np.zeros(3,3)
		mat[0, 1:] = (-(JV / s) * h.transpose()).transpose()
		mat[0,0] = 1 - N0/N
		mat[1:, 1:] = JV / p0
		M0 = M[:, i0]
		dpds_dy0 = mat * ((M0 / N) / H[0])
		dy0_dy = np.zeros((K0, K))
		dy0_dy[:, i0] = np.identity(K0)
		dpds_dy = dpds_dy0 * dy0_dy
		dpds_dy = pd.DataFrame(dpds_dy, index=['p', 'd', 's'])
		return dpds_dy
	xstd = (x - d) / s
	U = np.matrix([[xstd - H[1]/H[0]], xstd*xstd - H[2]/H[0]]])
	for i in range(len(M[:,0])):
		if i not in i0:
				M[:, i] = 0
	dl0plus_dy = U * (JV / s)
	dl0plus_dy = np.matrix([[1 - N0/N, dl0plus_dy[0,0], dl0plus_dy[0,1]]]) * M / (N * H[1] * p0)
	G = X.transpose() * (f * X)
	dl_dy = X * np.linalg.inv(G) * X.transpose()
	dlfdr_dy = dl0plus_dy - dl_dy
	if sw == 3:
		return dlfdr_dy
	Cov_lfdr = dlfdr_dy * (f * dlfdr_dy.transpose())
	return Cov_lfdr
